...
export const PipelineToolbar = () => {
  ...
  return (
    <Card className="bg-card rounded-b-none py-3 px-4 border-0">
      <div className="flex items-center m justify-between gap-4">
        <div className="flex items-center gap-6">
          <div className="flex-shrink-0 w-16 h-16 text-white flex justify-center items-center font-semibold bg-violet-800 text-4xl rounded-lg">
            <span>VS</span>
          </div>

        <div className="hidden lg:grid grid-flow-col px-40 auto-cols-[minmax(60px,1fr)] gap-2 items-center ">
            {nodes.map((node) => (
              <DraggableNode
                key={node.type}
                className="w-full"
